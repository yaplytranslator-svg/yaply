...
# ════════════════════════════════════════════════════════════════
#  GOOGLE OAUTH — FIXED (/auth/google was returning 403)
#  Root cause: route was missing + limiter was blocking it
# ════════════════════════════════════════════════════════════════

@app.route('/auth/google')
@limiter.exempt                           # ← CRITICAL: exempt from rate limiter
def google_login():
...
def google_callback():
    ...
    try:
        ...
        # Redirect to app with token — frontend stores it in localStorage
        return redirect(f'/app?token={token}&name={name.split()[0]}')
